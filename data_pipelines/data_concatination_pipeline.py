...
if __name__ == '__main__':
    ...
    for s3_folder in FOLDERS_TO_CONCAT:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            concated_files_list_path = ALREADY_CONCATED_FILES_LIST.format(spider_name=spider_name)
            ...
            log.info(f'Updated already concated list with {len(paths_to_concat)} new filepaths.')

            for path in paths_to_concat:
                bucket.download_file(path)
            concated_files = bucket.list_paths(directory = s3_target_dir,
                                                allowed_extension='.parquet')

            previous_concated = utils.select_most_up_to_date_file(concated_files)
            if len(previous_concated)>0:
                log.info('Selected most up-to-date file from previously concated: %s', previous_concated)
                bucket.download_file(previous_concated)
            else:
                log.info('Did not find previously concatinated files.')

            local_paths = [f'{tmpdir}/{utils.name_from_path(path)}' for path in paths_to_concat]
