...
class S3Bucket(object):
    ...
    def __init__(self, bucket_name, s3_dir_path, local_downloads_dir=None, profile=None):
        if profile:
    ...
    def upload_file(self, local_file, target_name):
        log.info(f'Uploading {local_file} as {target_name} to {self.bucket_name}...')
        with open(local_file, 'rb') as f:
            try:
                self.data_bucket.put_object(Key=target_name, Body=f)
            except Exception as e:
                log.warning(e)
